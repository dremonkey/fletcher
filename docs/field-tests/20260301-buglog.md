...
**Fix:**
Scope transcript state per-stream instead of sharing it across all concurrent streams. Each `OpenClawChatStream` should:
1. Receive its own segment ID at creation time (assigned by `onPondering(phrase)`)
2. Own its own `accumulatedContent` (already exists in `run()` but `lastTranscriptText` in agent.ts shadows it)
3. Only finalize its OWN segment in its `finally` block, not whatever the current global ID happens to be

Concretely: move from shared closures to a per-stream approach where `onPondering(phrase)` returns or captures the segment ID for that stream, and `onContent`/finalization use that captured ID rather than the global.
