 ...
 */
export function extractSessionFromContext(chatCtx, connOptions) {
    const session = {};
...
class OpenClawChatStream extends LLMStream {
    ...
    _onContent;
    _streamId;
    constructor(llmInstance, client, { chatCtx, toolCtx, connOptions, sessionKey, onPondering, onContent, streamId, }) {
    ...
    async run() {
        ...
        for (const item of chatCtx.items) {
            dbg.openclawStream('item type=%s instanceof ChatMessage=%s FunctionCall=%s', item?.constructor?.name, item instanceof ChatMessageClass, item instanceof FunctionCallClass);
        ...
        try {
            ...
            if (messages.length > 0) {
                const lastMsg = messages[messages.length - 1];
                dbg.openclawStream('last message: role=%s content="%s"', lastMsg.role, (lastMsg.content || '').slice(0, 100));
            }
            ...
            for await (const chunk of stream) {
                ...
                if (chunkCount <= 3 || delta.tool_calls) {
                    dbg.openclawStream('chunk %d: role=%s content="%s" hasToolCalls=%s', chunkCount, delta.role, (delta.content || '').slice(0, 50), !!delta.tool_calls);
                }
