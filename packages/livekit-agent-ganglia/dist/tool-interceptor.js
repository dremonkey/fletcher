 ...
 * ```
 */
import { statusFromToolCall, } from './events.js';
/**
 ...
 */
function matchesToolName(toolName, ...names) {
 ...
 */
export function createReadFileArtifact(toolCall, result) {
 ...
 */
export function createEditArtifact(toolCall, _result) {
    const filePath = extractFilePath(toolCall.args);
    if (!filePath) {
        return undefined;
    }
 ...
 */
export function createSearchArtifact(toolCall, result) {
    if (!result.success) {
        return undefined;
    }
    const query = toolCall.args.pattern ||
        toolCall.args.query ||
        toolCall.args.path ||
        '';
 ...
 */
export function createErrorArtifact(toolCall, error, stack) {
    return {
        type: 'artifact',
        artifact_type: 'error',
        message: error,
 ...
 */
export function createArtifactFromToolResult(toolCall, result) {
    // Handle errors
    if (!result.success) {
    ...
    async execute(toolCall, executor) {
        ...
        try {
            ...
            if (this.emitArtifacts) {
                const errorArtifact = createErrorArtifact(toolCall, errorMessage, errorStack);
 ...
 */
export function createToolInterceptor(config) {
