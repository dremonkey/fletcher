...
- [x] Each agent presence state has distinct visual feedback — color-coded system event cards (gray/green/red)
- [x] System events appear in chat transcript for lifecycle transitions — 4 transition types wired
- [x] "Speak to summon" hint is clear but not annoying — "Disconnected — speak to reconnect"
- [x] Dispatching state visible even for brief (<500ms) transitions — "Connecting..." event persists in transcript
- [x] Idle warning shows countdown — pushed from voice-agent via data channel with disconnectInMs
- [x] Transitions feel smooth, not jarring — AnimatedSwitcher on SystemEventCard
