...
- [ ] After SDK reconnect, `session/bind` is re-sent to the relay
- [ ] Prompt timeout (30s) surfaces an error to the user instead of hanging forever
- [ ] `session/bind` retries up to 3 times with 10s intervals
- [ ] Relay sends error notification to mobile when ACP subprocess dies
- [ ] Relay stops bridge after 3 consecutive forward failures
- [ ] Existing reconnect behavior (voice mode buffer, audio refresh) unchanged
