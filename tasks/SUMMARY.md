...
- [x] 092: Background Resume Token Retry ✅ — `_reconnectAfterBackground()` retries up to 3 times with exponential backoff (3s, 6s); waits for network readiness; engages `tryReconnect()` as fallback; cancellable on re-background; 9 unit tests ([BUG-044](../docs/field-tests/20260316-buglog.md))
- [ ] 096: Cold Start Connection Failure ⚠️ — RCA for BUG-049; `connectWithDynamicRoom()` races URLs before network routes are functional; `tryReconnect()` dead with null credentials; fix: fallback to fresh connect + network readiness wait ([BUG-049](../docs/field-tests/20260317-buglog.md))
- [x] 097: Prompt Timed Out UX Bug ✅ — inactivity timer replaces wall-clock timeout; resets on every incoming event ([BUG-050](../docs/field-tests/20260317-buglog.md))
- [x] 098: Surface Tool Calls in Chat Mode ✅ — `verbose_level` negotiation fixed; `option.id` fallback; tool events now flow to mobile ([BUG-051](../docs/field-tests/20260317-buglog.md))
- [ ] 099: Relay Process Supervision ⚠️ — RCA complete: 3 shutdowns (SIGTERM from TUI/monitor, hard crash likely OOM/segfault); SIGHUP not handled; fix: Dockerize with `restart: unless-stopped` ([BUG-054](../docs/field-tests/20260317-buglog.md))
- [ ] 100: Re-introduce Deferred Teardown ⚠️ — RCA complete: `participant_left` webhook immediately destroys bridge+ACP; deferred teardown infra exists; highest-leverage fix for network switch ([BUG-052](../docs/field-tests/20260317-buglog.md))
...
- [x] 095: Session Load Partial or Missing ✅ — Added `user_message_chunk` parsing, decoupled history load from room staleness, `_connectToNewRoom()` sets `_needsSessionLoad`, error handling for `_loadSessionHistory()`, replay logging for dropped update kinds ([BUG-047](../docs/field-tests/20260316-buglog.md))
...
- [x] T29.05: Verify tool-call pulse events (`tool_call`, `tool_call_update`) during multi-tool conversations
- [x] T29.06: Verify reasoning stream delivery (`agent_thought_chunk`)
