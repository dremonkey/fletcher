...
- [ ] 019: Internal Memory Leak Mitigations 📋 — `TranscriptManager.knownStreamIds` cleanup, OTel span leak patch (`bun patch`), Docker memory limit
...
- [ ] 019: Background Session Timeout & App-Close Disconnect 📋 — 10-min timeout on app-switch (not lock screen), immediate disconnect on swipe-away, to reduce battery drain from always-on foreground service
