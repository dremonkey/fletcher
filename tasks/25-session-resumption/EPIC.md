...
- [ ] **TASK-081: Session key schema + client→relay protocol** -- Decouple session key from room name. Shared word pair naming: room = `adj-noun-4chr`, session = `adj-noun-YYYYMMDD`. Client sends `session/bind` via data channel; relay defers ACP spawn until bind. Foundation for both resumption and multi-session.
