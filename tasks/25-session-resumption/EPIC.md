...
- [x] **TASK-081: Session key schema + client→relay protocol** -- Decouple session key from room name. Shared word pair naming: room = `adj-noun-4chr`, session = `adj-noun-YYYYMMDD`. Client sends `session/bind` via data channel; relay defers ACP spawn until bind. 30s bind timeout. 10 relay tests + 50 mobile tests.
